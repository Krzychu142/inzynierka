...
class AuthService {
  ...
  private static emailAdress =
    process.env.EMAIL_ADRESS || 'krzysztofradzieta@outlook.com'
  ...
  private static async sendEmail(
    ...
    const emailOptions = emailSender.emailOptions(
      this.emailAdress,
      recipientEmail,
      subject,
      message,
  ...
  static async register(userData: IEmployee): Promise<IEmployee | string> {
    try {
      ...
      userData.passwordResetToken =
        TokenService.generateTokenForEmailVerificationOrPasswordReset(
          userData.email,
        )

      const user = new Employee(userData)
      await user.save()
      const encodedToken = Buffer.from(userData.passwordResetToken).toString(
        'base64',
      )
  ...
  static async verifyEmail(token: string): Promise<void> {
    try {
      const decodedToken =
        TokenService.verifyTokenForEmailVerificationOrPasswordReset(
          token,
        ) as JwtPayload
  ...
  static async forgotPassword(email: string): Promise<void> {
    try {
      ...
      const encodedToken = Buffer.from(user.passwordResetToken).toString(
        'base64',
      )
